...
Form::Form() : name("Form's name"), gradeToSign(150), executedGrade(150){
    this->sign = false;
}
...
Form::Form(const std::string name, int gradeToSign, int executedGrade): name(name), gradeToSign(gradeToSign), executedGrade(executedGrade) {
...
Form::Form(const Form& obj): sign(obj.sign), gradeToSign(obj.gradeToSign), executedGrade(obj.executedGrade), name(obj.name){
